...
#(Intercept)     -0.09751    1.21083  -0.081   0.9358    
#FloatY           0.81447    0.43824   1.859   0.0631 .  
#TreatmentFresh   6.35464    1.06185   5.984 2.17e-09 ***
#TreatmentSalt    5.81797    0.81183   7.166 7.70e-13 ***
#TreatmentSoil    3.56103    0.44185   8.059 7.67e-16 ***
#PropaguleLength -0.90716    0.55774  -1.626   0.1038    
#---
#Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1

#(Dispersion parameter for binomial family taken to be 1)

#    Null deviance: 405.89  on 357  degrees of freedom
#Residual deviance: 176.85  on 352  degrees of freedom
#AIC: 188.85
...
#      0.9070952       2.2579843     575.1558966     336.2878650      35.1996004       0.4036688
...
```{r}
#here I am creating a new dataframe to hold the predicted values, I am keeping length constant at the mean length value, so predicted values only reflect treatment and bouyancy, expand.grid creates all combos of supplied vectors (my float and treatment vectors) keeping propagule length constant at the mean
...
# Mass sequence (100 points across observed range)
mass_seq <- seq(
  from = min(LARA$Mass),
  to   = max(LARA$Mass),
  length.out = 100
...
# Add fit and SE to newdat
LARApredict$fit <- pred$fit
LARApredict$se <- pred$se.fit

# 95% confidence interval on link scale, then back-transform to probability
...
To cleanup code, this doesnt currently work want to make a function to run all the logisitic regressions, then can run for all poissons/logistic regressions


```{r}

explanitorylist<-c("Treatment","Treatment + SiteID","Treatment + Buoyancy", "Treatment + Length","Treatment + Width", "Treatment + Mass", "Treatment + Buoyancy + Length", "Treatment + Buoyancy + Width","Treatment + Buoyancy + Mass","Treatment + SiteID + Buoyancy","Treatment + SiteID + Buoyancy + Length","Treatment + SiteID + Buoyancy + Width", "Treatment + SiteID + Buoyancy + Mass")

#writing a function to loop through all of these and put them as 
function(list,responseVAR, data, model){
  for ()
  glm(response~[i] )
  
}
run_logistic_models <- function(data, response) {
  
  # List of model formulas as strings
  model_formulas <- c(
    "Treatment",
    "Treatment + SiteID",
    "Treatment + Buoyancy",
    ...
    "Treatment + Width",
    "Treatment + Mass",
    "Treatment + Buoyancy + Length",
    "Treatment + Buoyancy + Width",
    "Treatment + Buoyancy + Mass",
    "Treatment + SiteID + Buoyancy",
    "Treatment + SiteID + Buoyancy + Length",
    "Treatment + SiteID + Buoyancy + Width",
    "Treatment + SiteID + Buoyancy + Mass"
  )
  
  # Initialize lists to store models and AICs
  models <- list()
  aic_values <- numeric(length(model_formulas))
  
  # Fit each model
  for (i in seq_along(model_formulas)) {
    f <- as.formula(paste(response, "~", model_formulas[i]))
  ...
  aic_table$Weight <- exp(-0.5 * aic_table$Delta_AIC) /
    sum(exp(-0.5 * aic_table$Delta_AIC))
  
  # Sort by AIC
  aic_table <- aic_table[order(aic_table$AIC), ]
  
  return(list(AIC_table = aic_table, Models = models))
}


```

sprouted 0/1, LARA logisitic regression best model included Table 6 
```{r LARA Only Logit}
library(DHARMa)#for simulate residuals, to check assumptions
