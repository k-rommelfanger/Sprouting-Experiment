...
Look at the total number sprouted per site per treatment

```{r}
LARAsprout<-LARA%>%
  group_by(SiteID, Treatment, sprouted)%>%
  summarise(N=n())%>%
...
#%>%
  filter(sprouted==1)%>%
  ggplot(aes(x=SiteID, y=N, fill=Treatment))+
...
Plot intake char vs sprout/notsprout see if it looks like there are differences in intake char of those that sprouted or didn't sprout
```{r}
...
Run a T test or Nonparametric test checking For differences between those that sprouted and those that didn't
...
Facet by treatment (This isnt useful)
...
sproutplot<-AVGE%>%
  ggplot(aes(x=sprouted, y=PropaguleLength))+
  geom_violin()+
   geom_sina()+
  geom_boxplot(width=0.1)+
 # geom_point(data=meanlenght, aes(x=sprouted, y=mean), color="red")+
  ggtitle("Propagule Length")+
...
sproutplotRadL<-AVGE%>%
  ggplot(aes(x=sprouted, y=`IntakeRadicleLength-cm`))+
  geom_violin()+
   geom_sina()+
  geom_boxplot(width=0.1)+
  geom_point(data=meanradlenght, aes(x=sprouted, y=mean), color="red")+
  ggtitle("Intake Rad Length")+
  facet_wrap(~Treatment)

sproutplotRadL

AVGEplot<-sproutplot+sproutplotRadL
AVGEplot
...
Calculating mean and sem of intake characteristics (MEAN/SEM Intake Variables Table 2)
...
Look at sprout or not sprout as a binary, 0/1 then plot and run logistic regression to see the effect of the prop characteristics on whether or not things sprouted AVGE, This Section was more exploratory, ran these models more systematically/organized below
...
#also going to look for correlation in intake radicle length
cor_matrix <- cor(AVGE[, c("PropaguleLength", "Width-mm", "Mass","IntakeRadicleLength-cm")], use = "complete.obs")
...
Visualized AVGE in two ways one just holding radL and L constant and generating results for float and not float, the other varying intake RL across observed values
...
want to visualize with range of values of intake radicle length and see which I like better

```{r}

AVGEpredictRad<- expand.grid(
  Float = c("Y", "N"), #two bouyancy options
  Treatment = c("Fresh", "Salt", "Soil", "Air"), #four treatment options
  PropaguleLength = mean(AVGE$PropaguleLength), #hold length constant
  `IntakeRadicleLength-cm`   = seq(min(AVGE$`IntakeRadicleLength-cm`),
               max(AVGE$`IntakeRadicleLength-cm`),
               length.out = 100))  # radicle lenght across values

# we are generating data for each treatment using the model created from our experimental data here, predict(mylogitmodel, newdf, type=return linear predictor log odds, not translate to probablitiles, also give SE)
AVGEpredictRad1 <- predict(NoSLengthLogitR, AVGEpredictRad, type = "link", se.fit = TRUE)

AVGEpredictRad$prob <- plogis(AVGEpredictRad1$fit)  # predicted probability
AVGEpredictRad$lower <- plogis(AVGEpredictRad1$fit - 1.96 * AVGEpredictRad1$se.fit)  # lower 95% CI
AVGEpredictRad$upper <- plogis(AVGEpredictRad1$fit + 1.96 * AVGEpredictRad1$se.fit)  # upper 95% CI



AVGEpredplot<-ggplot(AVGEpredictRad, aes(x = `IntakeRadicleLength-cm`, y = prob, color = Treatment, fill = Treatment)) +
  geom_line(linewidth =.5) +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.2, color = NA) +
  geom_point(data = AVGE, aes(x = `IntakeRadicleLength-cm` , y = sprouted, color=Treatment), height = 0.03, alpha = 0.3, inherit.aes = FALSE) +
  labs(x = "Propagule Radicle Length at Intake", y = "Predicted probability of sprouting") +
  ggtitle(expression(italic("A. germinans")~"Predicted Sprouting Probability"))+
  theme(plot.title = element_text(hjust = 0.5),panel.background = element_rect(fill = "white", colour = "grey50"))+
  scale_fill_manual(name="Treatment",values=c("cornflowerblue","darkblue","salmon4","azure4"))+
  scale_color_manual(name="Treatment",values=c("cornflowerblue","darkblue","salmon4","azure4"))+
  facet_wrap(~ Float, 
             labeller = labeller(
               Float = c(
                 "Y" = "Floated",
...
AVGEpredplot <- ggplot(
  ...
  labs(
    x = "Initial radicle length (cm)",
    y = "Predicted probability of sprouting",
    title = expression(italic("A. germinans") ~ "Predicted Sprouting Probability")
  ) +
  coord_cartesian(ylim = c(0, 1)) +
  scale_fill_manual(
    values = c("azure4", "cornflowerblue", "darkblue", "salmon4")
  ) +
  scale_color_manual(
    values = c("azure4", "cornflowerblue", "darkblue", "salmon4")
  ) +
...
Now plotting the best model using predict, these figures look different because while the best model for AVGE included lenght, bouyancy (categorical variable) had a stronger impact on the differences in sprouting, so I held length constant and had bouyancy be on the x axis, for the LARA model, we have mass (continuous variable) on the x axis 
...
LARApredict<- expand.grid(
  ...
  Mass = seq(min(AVGE$`IntakeRadicleLength-cm`),
               max(AVGE$`IntakeRadicleLength-cm`),
               length.out = 100))  # radicle lenght across values


# Predict values using my model, on link scale with standard errors calculated for each value
pred <- predict(TSM, newdata = LARApredict, type = "link", se.fit = TRUE)
...
What does it look like if i hold mass constant and have site and treatment?
...
LARAlogitplot<-ggplot(LARApredict1, aes(x = Treatment, y = pred_prob, color = SiteID)) +
  ...
  ggtitle(expression(italic("L. racemosa")~"Predicted Sprouting Probability"))+
   scale_color_manual(values = c("Vessup Salt Pond" = "olivedrab", "STEER" = "salmon4","Compass Point"="darkgreen"),
                    name = "Site ID", labels=c( "Vessup Salt Pond", "STEER","Compass Point"))+
  theme(plot.title = element_text(hjust = 0.5),panel.background = element_rect(fill = "white", colour = "grey50"))+
  scale_x_discrete(labels = c(
  "Fresh" = "Freshwater",
  "Salt" = "Saltwater",
...
unique(lubesprout$DaysUntilSprout)

#days until sprout as numeric
lubesprout$DaysUntilSprout<-as.numeric(lubesprout$DaysUntilSprout)

lubesprout<-lubesprout%>%
  filter(DaysUntilSprout!="NA")

unique(lubesprout$DaysUntilSprout)

#calculate average days until sprouting by treatment
lubecheck<-lubesprout%>%
...
#Visualizing data looking for patterns to see if any obvious affect on duration to sprouting
...
Kaidrian this is the section that I am working on rerunning, a Poisson regression is like a linear regression, but you use it on data that is not continuous. (Like if you are counting fish across several depths and want to look at the effect of depth on number of fish, you cant have 0.5 or 0.75 fish, you can only have, 1, 2,3.. fish whole numbers) Because days to sprouting are whole numbers, I thought that this would could work, thinking of days until sprouting as counts of the number of days. However there is a type of analysis called survival analysis/Kapelin Meyer Curves/cox regression and competing risks analysis/GreyFineAnalysis that is a better way to analyze this data

For the final publication code I will edit this out, but I think it might be useful for you to see the thought process
...
This is running poissons on AVGE data here, again rerunning the survival analysis instead 
...
this here is more poissons, in the chunks above i didnt run the models systematically enough, i missed some combinations of variables, so this was me going through and doing that before I decided to change this analysis 
...
#STEER basin AVGE  
SteerAVGEprop<-PropPlot(DPropGermSproutCount,"AVGE", "STEER", AS)

SteerAVGEprop

#STEER basin LARA
SteerLARAprop<-PropPlot(DPropGermSproutCount,"LARA", "STEER", LS)
