...
#going to remove these NAs and -1, removal date isn't noted for these
sprout<-sprout%>%
...
#ok data looks clean now, ok to leave a lot of these for binary analysis (sprout not sprout 0/1) but need to remove for time series analysis because dont know date of removal or sprouting
...
LARAcox<-coxph(Surv(time = LARA$DaysUntilEvent, event = LARA$sprouted)~Treatment+SiteID+Mass+Float, data=LARA)
