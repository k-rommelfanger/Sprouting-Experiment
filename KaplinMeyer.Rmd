...
totalpropsplot$ReasonForRemoval<-factor(totalpropsplot$ReasonForRemoval, levels=c("Not Noted","experiment end","dead","predation","mold","Sprouted"))
...
plotNotNoted<-ggcuminc(cra, outcome=c("5"))
plotNotNoted


#using this package gives me the same results as what chat taught me! score!
plottt<-plotchat+plottutorial
plottt

```


Now running FineGrey, this looks at the effect of different explanatory varibles on our outcome we choose 
```{r}
#CIF for sprouting
m1 <- crr(Surv(DaysUntilEvent, Fate) ~ Treatment + SiteID + Mass,
         failcode = "1",
         data = LARAcr)
...
#CIF for mold
m2<-crr(Surv(DaysUntilEvent, Fate) ~ Treatment + SiteID + Mass,
         failcode = "2",
         data = LARAcr)
m2
