...
removalplot
#make sure to export wider, so bars show up fatter and easier to see when actually ggsave these!
...
Austin, P. C., & Fine, J. P. (2017). Practical recommendations for reporting F ine‐G ray model analyses for competing risk data. Statistics in medicine, 36(27), 4391-4400.


#another source for competing risks (good plain language descr)
...
LARAcr <- LARAcr %>%
  mutate(Treatment = factor(
    Treatment,
    levels = c("Fresh", "Salt", "Soil", "Air"),
    labels = c("Freshwater", "Saltwater", "Soil", "No Substrate")
  ))
LARAcr$SiteID<-as.factor(LARAcr$SiteID)
LARAcr$Fate<-as.factor(LARAcr$Fate)
cra<-tidycmprsk::cuminc(Surv(DaysUntilEvent, Fate) ~ Treatment, data = LARAcr)


LARAplotsprout<-ggcuminc(cra, outcome = c("1"))+
  add_confidence_interval(color=NA)+
  add_risktable()+
  scale_color_manual(name="Treatment",values=c("cornflowerblue","olivedrab","darkblue","salmon4"))+
  scale_fill_manual(name="Treatment",values=c("cornflowerblue","olivedrab","darkblue","salmon4"))+
   ggtitle(expression(italic("L. racemosa")))+
...
plotmold<-ggcuminc(cra, outcome=c("2"))+
  ...
  scale_fill_manual(name="Treatment",values=c("olivedrab","salmon4","darkblue","salmon4"))
...
plotting separtely for each site LARA
```{r}
cuminc_list <- LARAcr %>%
  group_by(SiteID) %>%
  group_map(~ tidycmprsk::cuminc(Surv(DaysUntilEvent, Fate) ~ Treatment, data = .x), .keep = TRUE)


plots <- lapply(seq_along(cuminc_list), function(i) {
  ggcuminc(cuminc_list[[i]], outcome = "1") +
    add_confidence_interval(alpha = 0.2) +
    scale_color_manual(name = "Treatment",
                       values = c("azure4","cornflowerblue","darkblue","salmon4")) +
    scale_fill_manual(name = "Treatment",
                      values = c("azure4","cornflowerblue","darkblue","salmon4")) +
    ggtitle(paste0("Site: ", levels(LARAcr$SiteID)[i])) +
    theme(plot.title = element_text(hjust = 0.5))
})
...
Plotting separately for each site AVGE
```{r}

AVGEcr$Float<-as.factor(AVGEcr$Float)
cuminc_list <- AVGEcr %>%
  group_by(Float) %>%
  group_map(~ tidycmprsk::cuminc(Surv(DaysUntilEvent, Fate) ~ Treatment, data = .x), .keep = TRUE)
...
Acra<-tidycmprsk::cuminc(Surv(DaysUntilEvent, Fate) ~ Treatment, data = AVGEcr)
